...
//=============================================================================================================================
void TM_SPI_Send(unsigned char data){
	unsigned char  bit=0;

    for( bit = 0x80; bit; bit >>= 1) {
		   
      
      if(data & bit){SSD1306_SDO_H();}
      else{ SSD1306_SDO_L();
	  }
...
//=============================================================================================================================
void TM_SPI_Send_Buff(unsigned char *data,int size)
{ 
int i;
for(i=0;i<size;i++)
	  TM_SPI_Send(data[i]);
...
//============================================================================================================================= 
 void SSD1306_cmd_double(uint8_t cmd1, uint8_t cmd2) {
	// Deassert DC pin -> command transmit
	SSD1306_DC_L();
...
void SSD1306_InitGPIO(void) {
	...
	GPIO_InitTypeDef PORT;
  	RCC_APB2PeriphClockCmd(SSD1306_GPIO_PERIPH,ENABLE);
	
	PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	PORT.GPIO_Pin = SSD1306_CS_PIN;
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SSD1306_CS_PORT,&PORT);
	SSD1306_CS_H();

		PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	PORT.GPIO_Pin = SSD1306_DC_PIN;
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SSD1306_DC_PORT,&PORT);
	SSD1306_DC_L();

	PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	PORT.GPIO_Pin = SSD1306_RST_PIN;
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SSD1306_RST_PORT,&PORT);
	SSD1306_DC_L();

	PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	PORT.GPIO_Pin = SSD1306_SDO_PIN;
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SSD1306_SDO_PORT,&PORT);

	PORT.GPIO_Mode = GPIO_Mode_Out_PP;
	PORT.GPIO_Pin = SSD1306_CLK_PIN;
	PORT.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SSD1306_CLK_PORT,&PORT);
...
//	SPIx_SendBuf(&SSD1306_SPI_PORT,(uint8_t *)SSD1306_SET_ADDR_0x0,sizeof(SSD1306_SET_ADDR_0x0));
   TM_SPI_Send_Buff( (uint8_t *)SSD1306_SET_ADDR_0x0,sizeof(SSD1306_SET_ADDR_0x0));
	// Assert DC pin -> data transfer
	SSD1306_DC_H();

	// Transmit video buffer to LCD
//	SPIx_SendBuf(&SSD1306_SPI_PORT,vRAM,(SCR_W * SCR_H) >> 3);
	TM_SPI_Send_Buff(vRAM,(SCR_W * SCR_H) >> 3);
...
//=============================================================================================================================
uint8_t LCD_PutChar(uint8_t X, uint8_t Y, uint8_t Char, const Font_TypeDef *Font, uint8_t *font_Data) {
	uint8_t pX;
	uint8_t pY;
	uint8_t tmpCh;
	uint8_t bL;
	...
	pCh =&font_Data[(Char - Font->font_MinChar)  * Font->font_BPC];
	 // pCh = font_Data;
...
//=============================================================================================================================
	uint16_t LCD_PutStr(uint8_t X, uint8_t Y, const char *str, const Font_TypeDef *Font, uint8_t* fontdata) {
	uint8_t pX = X;
	uint8_t eX = scr_width - Font->font_Width - 1;

	while (*str) {
		pX += LCD_PutChar(pX,Y,*str++,Font,fontdata);
		if (pX > eX) break;
	}
...
uint16_t LCD_PutStrLF(uint8_t X, uint8_t Y, const char *str, const Font_TypeDef *Font,uint8_t* fontdata) {
	uint8_t strLen = 0;

    while (*str) {
        LCD_PutChar(X,Y,*str++,Font, fontdata);
...
//=============================================================================================================================
uint8_t LCD_PutInt(uint8_t X, uint8_t Y, int32_t num, const Font_TypeDef *Font,uint8_t *fontData) {
	...
	*pStr++ = '\0';
	...
	// Draw a number
	while (*--pStr) pX += LCD_PutChar(pX,Y,*pStr,Font,fontData);
...
// return: number width in pixels
//=============================================================================================================================
uint8_t LCD_PutIntU(uint8_t X, uint8_t Y, uint32_t num, const Font_TypeDef *Font, uint8_t *fontData) {
...
uint8_t LCD_PutIntF(uint8_t X, uint8_t Y, int32_t num, uint8_t decimals, const Font_TypeDef *Font, uint8_t *fontData) {
	uint8_t str[11]; // 10 chars max for INT32_MIN..INT32_MAX (without sign)
	uint8_t *pStr = str;
	uint8_t pX = X;
	uint8_t neg = 0;
	...
	*pStr++ = '\0';
	...
	// Draw a number
	while (*--pStr) {
		pX += LCD_PutChar(pX,Y,*pStr,Font,fontData);
...
// return: number width in pixels
//=============================================================================================================================
uint8_t LCD_PutIntLZ(uint8_t X, uint8_t Y, int32_t num, uint8_t digits, const Font_TypeDef *Font, uint8_t *fontData) {
	uint8_t str[11]; // 10 chars max for INT32_MIN..INT32_MAX (without sign)
	uint8_t *pStr = str;
	uint8_t pX = X;
	uint8_t neg = 0;
...
// return: number width in pixels
//=============================================================================================================================
uint8_t LCD_PutHex(uint8_t X, uint8_t Y, uint32_t num, const Font_TypeDef *Font,uint8_t *fontData) {
	uint8_t str[11]; // 10 chars max for UINT32_MAX
	uint8_t *pStr = str;
	uint8_t pX = X;
